...
def GameOver(gamesurface, time_start, time_end):
    ...
    with open("data/mouse_data.csv", 'a+') as f:
...
def game(ftpsClock):
    ...
    keyboard_color = keyboard_font.render(u'鼠标版', True, grey_colour)
    ...
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                tmp_data = [derection, snakeposition, [mouse_x, mouse_y]]
                data_list.append(tmp_data)
                if derection == 'right' or derection == 'left':
                    if mouse_y < snakeposition[1]:
                        change_derection = 'up'
                    else:
                        change_derection = 'down'
                if derection == 'up' or derection == 'down':
                    if mouse_x < snakeposition[0]:
